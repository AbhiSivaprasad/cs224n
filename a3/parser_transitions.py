...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            # left arc removes second to last element, right arc removes last element
            element = self.stack.pop(-2) if transition == "LA" else self.stack.pop(-1)

            # form dependency (top element of stack, popped element)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for unfinished_parse, transition in zip(minibatch, transitions):
            unfinished_parse.parse_step(transition)
        
        is_finished_parse = lambda parse: len(parse.buffer) == 0 and len(parse.stack) == 1
        unfinished_parses = [parse for parse in unfinished_parses if not is_finished_parse(parse)]
