...
class ParserModel(nn.Module):
    ...
    def forward(self, w):
        ...
        input_tensor = self.embedding_lookup(w)
        hidden = F.relu(torch.mm(input_tensor, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        hidden_dropout = self.dropout(hidden)
        logits = torch.mm(hidden_dropout, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
